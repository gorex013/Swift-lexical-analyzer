...
#       pragma
clang diagnostic
ignored "-Wswitch-enum"
#       pragma
clang diagnostic
ignored "-Wcovered-switch-default"
#    endif
#elif
defined __GNUC__
...
#if
defined(CATCH_CONFIG_MAIN)
||

defined(CATCH_CONFIG_RUNNER)
...
#if
defined(CATCH_INTERNAL_CONFIG_COUNTER) &&
...
#endif
#if
        defined(CATCH_INTERNAL_CONFIG_WINDOWS_SEH)
&& !
...
#if
defined(CATCH_INTERNAL_CONFIG_POSIX_SIGNALS) &&
...
#if
!
defined(CATCH_INTERNAL_CONFIG_NO_WCHAR)
&& !
...
#if
!
defined(CATCH_INTERNAL_CONFIG_NO_CPP11_TO_STRING)
&& !
...
#if
defined(CATCH_INTERNAL_CONFIG_NEW_CAPTURE) &&
...
#if
!
defined(CATCH_INTERNAL_CONFIG_EXCEPTIONS_ENABLED)
&& !
...
#define
INTERNAL_CATCH_EXPAND2(...) INTERNAL_CATCH_NO##
...
namespace {
    ...
    struct TestName : INTERNAL_CATCH_EXPAND1(INTERNAL_CATCH_DEF
    ClassName) {
    \
...
namespace {
    ...
    (
    Catch::makeTestInvoker( &TestName ), CATCH_INTERNAL_LINEINFO, Catch::StringRef(), Catch::NameAndTags{
            __VA_ARGS__
    }
    );
} /* NOLINT */ \
...
CATCH_INTERNAL_SUPPRESS_GLOBALS_WARNINGS \
        Catch::AutoReg
INTERNAL_CATCH_UNIQUE_NAME( autoRegistrar )(Catch::makeTestInvoker(Function), CATCH_INTERNAL_LINEINFO,
...
#pragma
warning(disable
:4180) // We attempt to stream a function (address) by const&, which MSVC complains about but is harmless
#endif
...
namespace Catch {
    ...
    static std::string convert(const std::tuple<Types...> &tuple) {
        ReusableStringStream rss;
        rss << '{';
        Detail::TupleElementPrinter<std::tuple < Types...>>
        ::print(tuple, rss.get());
        rss << " }";
        return rss.str();
    }
...
static std::string convert(std::chrono::duration < Value, std::ratio < 1 >>
const &duration) {
ReusableStringStream rss;
rss << duration.
...
static std::string convert(std::chrono::duration < Value, std::ratio < 3600 >>
const &duration) {
ReusableStringStream rss;
rss << duration.
...
static std::string convert(std::chrono::time_point < Clock, Duration >
const &time_point) {
return
::Catch::Detail::stringify(time_point
.
...
static std::string convert(std::chrono::time_point < std::chrono::system_clock, Duration >
const &time_point) {
...
#pragma
warning(disable
:4018) // more "signed/unsigned mismatch"
#pragma
warning(disable
...
#if
defined(CATCH_CONFIG_FAST_COMPILE)
||
defined(CATCH_CONFIG_DISABLE_EXCEPTIONS)
...
##_catch_sr, CATCH_INTERNAL_LINEINFO,
...
Catch::StringRef(), resultDisposition
        ...
        catchAssertionHandler.
handleMessage( messageType, ( Catch::MessageStream()
<< __VA_ARGS__ +::Catch::StreamEndStop()).m_stream.
...
(
Catch::MessageBuilder( macroName
##_catch_sr, CATCH_INTERNAL_LINEINFO, Catch::ResultWas::Info ) << log );
...
INTERNAL_CATCH_SECTION(...) \
    CATCH_INTERNAL_SUPPRESS_UNUSED_WARNINGS \
    if (
Catch::Section const &INTERNAL_CATCH_UNIQUE_NAME(catch_internal_Section)
=
...
Catch::Section const &INTERNAL_CATCH_UNIQUE_NAME(catch_internal_Section)
=
Catch::SectionInfo( CATCH_INTERNAL_LINEINFO, (Catch::ReusableStringStream()
<< __VA_ARGS__).
...
UnorderedEqualsMatcher(std::vector < T >
const &target) :
m_target(target) {
}
...
if (
Detail::contains(lfirst, mid, *mid
)) {
continue;
}
size_t num_vec = Detail::count(rfirst, rlast, *mid);
if (num_vec == 0 ||
Detail::count(lfirst, llast, *mid
) != num_vec) {
return false;
}
}
...
template<typename T>
Vector::ContainsMatcher <T> Contains(std::vector < T >
const &comparator) {
return
Vector::ContainsMatcher<T>(comparator);
}
...
template<typename T>
Vector::EqualsMatcher <T> Equals(std::vector < T >
const &comparator) {
return
Vector::EqualsMatcher<T>(comparator);
}

template<typename T>
Vector::UnorderedEqualsMatcher <T> UnorderedEquals(std::vector < T >
const &target) {
return
Vector::UnorderedEqualsMatcher<T>(target);
}
...
\
            catchAssertionHandler.
handleExpr( Catch::makeMatchExpr(arg, matcher,
#
...
\
                catchAssertionHandler.
handleExpr( Catch::makeMatchExpr(ex, matcher,
#
...
type (( Catch::ReusableStringStream()
...
Catch::throw_exception(CATCH_PREPARE_EXCEPTION(std::logic_error,
...
[]{
using namespace Catch::Generators; return
makeGenerators( __VA_ARGS__ );
} )
...
-(void)
OC_MAKE_UNIQUE_NAME( Catch_TestCase_test_, uniqueSuffix
)
...
namespace {
    Catch::ReporterRegistrar<reporterType> catch_internal_RegistrarFor
    ##
    reporterType( name );
}
\
...
defined(NOMINMAX)
&& !
defined(CATCH_CONFIG_NO_NOMINMAX)
#  define
...
namespace Catch {
    ...
    struct FatalConditionHandler {
        ...
        handleVectoredException(PEXCEPTION_POINTERS
        ExceptionInfo);
...
#endif
#define
CATCH_CLARA_TEXTFLOW_CONFIG_CONSOLE_WIDTH CATCH_CONFIG_CONSOLE_WIDTH
-1
...
#pragma
clang diagnostic
ignored "-Wexit-time-destructors"
#pragma
clang diagnostic
ignored "-Wshadow"
#endif
...
// the actual error message will be in m_errorMessage
        ...
        auto setValue(std::string const &arg) -> ParserResult
...
// In Objective-C try objective-c exceptions first
            ...
            {
                return Catch::Detail::stringify([exception
                description] );
            }
#else
...
namespace Catch {
    ...
    namespace {
        ...
        IStreamingReporterPtr makeReporter(std::shared_ptr < Config >
        const &config) {
        if (
    ...
    ->
    create(Catch::ReporterConfig(config)
    ));
}
multi->
addReporter(createReporter(config->getReporterName(), config)
);
return
std::move(multi);
}

Catch::Totals runTests(std::shared_ptr < Config >
const &config) {
...
.
noMatchingTestCases(testConfig
.
...
// already in declaration order
    ...
    << "\tFirst seen at " << prev.first->

    getTestCaseInfo()

    .lineInfo << "\n"
    << "\tRedefined at " << function.
    ...
    namespace {
        auto estimateClockResolution() -> uint64_t {
            ...
            for (std::size_t i = 0; i < iterations; ++i) {
                ...
                ticks;
                uint64_t
                baseTicks = getCurrentNanosecondsSinceEpoch();
                do {
...
#define
    CATCH_TEST_CASE_METHOD(className, ...)
...
#define
    CATCH_REGISTER_TEST_CASE(Function, ...)
...
#define
    ...
    __VA_ARGS__ )
#define
    CATCH_SCENARIO_METHOD(className, ...)
    INTERNAL_CATCH_TEST_CASE_METHOD(className, "Scenario: "
    __VA_ARGS__ )
#define
    CATCH_GIVEN(desc)
